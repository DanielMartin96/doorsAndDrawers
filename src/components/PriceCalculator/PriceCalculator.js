...
const PriceCalculator = ({ style }) => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault()
    if (isStandard(height, heights) && isStandard(width, widths)) {
      findPrice(height, width, prices)
      console.log(findPrice(height, width, prices).price)
    }
    if (isStandard(height, heights) && !isStandard(width, widths)) {
      const nextWidth = await findNextSizeUp(width, widths);
      console.log(await findPrice(height, nextWidth, prices))
    }
  }
